...
class EditView implements View {
  ...
  void _saveDetails(Expense expense) {
    ...
    //print('before sync item: $expense');
    app.syncItem(expense);
  }
...
void _addActionEditCheckBox(StringBuffer result) {
  ...
  result.write(
      '<label><input type="checkbox" name="action" disabled checked value="${item.name}">'
      '${item.name.substring(0, 1).toUpperCase()}${item.name.substring(1, item.name.length)}</label><br>');
}
...
void _buildCheckBoxContent(StringBuffer result, String checked, Action item) {
  result.write(
      '<label><input type="checkbox" name="action" $checked value="${item.name}">'
      '${item.name.substring(0, 1).toUpperCase()}${item.name.substring(1, item.name.length)}</label><br>');
}
