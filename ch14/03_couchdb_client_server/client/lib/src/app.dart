...
class AppController {
  static const websocket_url = 'ws://127.0.0.1:8088/websocket';
  ...
  void buildUI() {
    ...
    final footer =
        Element.html('<footer class="section" id="footer"></footer>');
  ...
  void syncItem(Expense expense) {
    ...
    final data = {
      'action': 'SYNC',
      // we have  to prepare data for JSON encoding (by making a map)
  ...
  void connectToWebsocket() {
    _webSocket = WebSocket(websocket_url);
    _webSocket.onMessage.listen((MessageEvent message) {
      ...
      switch (dataMap['action']) {
        case 'CLIENT_COUNT_REFRESH':
          _conectedClients = dataMap['connectedClients'];
          refreshFooterStatus();
          break;
        case 'SYNC':
          final Map expenseMap = dataMap['expense'];
        ...
        case 'LOAD':
          print('LOAD');
          int loadedNextId = dataMap['nextId'];
          if (Expense.currentHighestId < loadedNextId) {
            Expense.currentHighestId = dataMap['nextId'];
          }

          for (Map expenseInfo in dataMap['expenses']) {
            var expense = Expense.fromMap(expenseInfo);
            _appData.addOrUpdate(expense);
          }
          // if you have data in the local storage
          // you can send them to the server
          // for synchronization

          List expenseInfoList = dataMap['expenses'];
          if (expenseInfoList.length <= appData.expenses.length) {
        ...
        default:
          throw UnimplementedError(
              'Action ${dataMap["action"]} is not implemented');
      }
    });
    _webSocket.onOpen.listen((Event event) {
      _webSocket.send('{"action":"LOAD"}');
    });
  }

  void refreshFooterStatus() {
    final statusText = '$_conectedClients clients connected';
    document.getElementById('footer').text = statusText;
  }
} //AppController
